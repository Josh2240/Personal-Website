...
export default function Projects() {
  ...
  return (
    <div className="w-full min-h-screen bg-background text-text-light overflow-hidden">
      <Header />
      <Navigation />
      <main className="mx-auto flex min-h-screen max-w-6xl flex-col gap-6 sm:gap-8 sm:gap-10 px-4 py-24 sm:px-6 lg:px-10 w-full">
        <section className="rounded-[32px] border border-white/10 bg-white/5 p-6 sm:p-8 shadow-[0_30px_100px_rgba(0,0,0,0.45)] backdrop-blur-xl">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
            <div>
              <p className="text-xs sm:text-sm uppercase tracking-[0.4em] text-[#9f9f9f]">Projects</p>
              <h1 className="mt-4 text-3xl sm:text-4xl font-bold uppercase tracking-[0.08em]">Recent Work</h1>
            </div>
        ...
        {loading ? (
          <div className="rounded-[32px] border border-white/10 bg-white/5 p-8 sm:p-10 text-center text-base sm:text-lg text-text-light shadow-[0_20px_60px_rgba(0,0,0,0.3)]">
            Loading projects...
          </div>
        ) : (
          <div className="grid gap-5 sm:gap-8 md:grid-cols-2">
            {projects.map((project) => (
              <article key={project.id} className="rounded-[24px] sm:rounded-[28px] border border-white/10 bg-white/5 shadow-[0_25px_80px_rgba(0,0,0,0.35)] transition hover:-translate-y-1 hover:border-white/20 hover:bg-white/10 overflow-hidden">
                {project.link ? (
                  <a href={project.link} target="_blank" rel="noopener noreferrer" className="block">
                    {project.image_url && (
                      <div className="relative h-32 sm:h-40 w-full bg-black/20">
